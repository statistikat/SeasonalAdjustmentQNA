...
told <- cbind(test.Type_more_info,told)
...
blaold <- as.data.frame(oldres$output$regarima$regression.coefficients)
blaold[order(substr(rownames(blaold),nchar(rownames(blaold))-5, nchar(rownames(blaold))-1 )),]
```
...
blanew <- as.data.frame(newres$output$regarima$regression.coefficients)
blanew[order(substr(rownames(blanew),nchar(rownames(blanew))-5, nchar(rownames(blanew))-1 )),]
```

:::
:::: 

Outliers from `oldres` not in `newres`:
```{r warning=FALSE,message=FALSE, echo=FALSE}
rownames(blaold)[which(!rownames(blaold) %in% rownames(blanew))]
```
Outliers from `newres` not in `oldres`:
```{r warning=FALSE,message=FALSE, echo=FALSE}
rownames(blanew)[which(!rownames(blanew) %in% rownames(blaold))]
```

Check if any outliers detected for `oldres` are new:
```{r warning=FALSE,message=FALSE, echo=FALSE}
source("get_new_outliers.R")
get_new_outliers(oldres)
```
Check if any outliers detected for `newres` are new:
```{r warning=FALSE,message=FALSE, echo=FALSE}
source("get_new_outliers.R")
...
Summary of diff between original and adjusted for `oldres`:
```{r warning=FALSE,message=FALSE, echo=FALSE}
summary(oldres$ts-oldres$adjusted)
```
Summary of diff between original and adjusted for `newres`:
```{r warning=FALSE,message=FALSE, echo=FALSE}
summary(newres$ts-newres$adjusted)
```
...
<!-- If the null hypothesis of no stable seasonality is rejected significance level < 0.01 (?? ambiguous in the literature) then the series is considered to be seasonal. \ -->
<!-- https://sylwiagrudkowska.github.io/JDemetra-documentation/pages/theory/Tests_combined.html -->

### Stable Seasonality:

The combined seasonality test in JDemetra+ consists of three tests:

- **Kruskal-Wallis test**
  - $H_0$: no stable seasonality
  - A small p-value indicates the presence of stable seasonality.
...
- **Evolutive seasonality test (moving seasonality test)**
  - $H_0$: the seasonal pattern is stable over time (no change in seasonality across years).
  - A large p-value indicates that the seasonality is stable over time.
  - A small p-value indicates evidence of changing (moving) seasonality.

A series is considered to exhibit identifiable seasonality when the first two tests reject the null hypothesis of no seasonality and there is no evidence of strong moving seasonality.
If the null hypothesis of no stable seasonality is rejected significance level < 0.01 (?? ambiguous in the literature) then the series is considered to be seasonal. \
https://sylwiagrudkowska.github.io/JDemetra-documentation/pages/theory/Tests_combined.html
https://sylwiagrudkowska.github.io/JDemetra-documentation/pages/theory/Tests_moving.html
...
https://github.com/rjdverse/rjdemetra/blob/90f8ff6e46e642e4c8fe064913118e19e6c96f7d/R/diagnostics.R#L9
