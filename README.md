...
The scripts `Step3_Bechnungen_T01xxx.R` derive adjusted series at cup and sums of aggregates. Series at cup are derived by multiplying adjusted vol series and deflators. Volume series are only defined for non-negative aggregates, so in those cases the series are adjusted at current prices. As volume series are not additive we need adjusted series at pyp to derive sums of aggregates. To derive them we follow the IMF Manual for QNA and use the seasonal factors of volume series. 
